...
class Goose(object):
    ...
    def extract(self, url=None, raw_html=None):
        ...
        """
        print("GOOSE__init__: extract: url :", url)
    ...
    def crawl(self, crawl_candidate):
        ...
        print ("goose.__init__: crawl_candidate : ", crawl_candidate.url, " crawl_candidate.raw_html :  ", crawl_candidate.raw_html )
        try:
            crawler = Crawler(self.config)
            article = crawler.crawl(crawl_candidate)
        except UnicodeDecodeError as u:
            print("goose.__init__.crawl: crawl_candidate :",crawl_candidate, "   UnicodeDecodeError", u)
            self.config.parser_class = parsers[0]
            return article
        except ValueError as v:
            print("goose.__init__.crawl: crawl_candidate : ValueError :", v)
            if "Unicode strings with encoding declaration are not supported" in repr(v):
                return None
            self.config.parser_class = parsers[0]
            return article
    ...
    def initialize(self):
        ...
        if not self.config.local_storage_path or \
           not self.config.enable_image_fetching:
            return
        # testdata if config.local_storage_path
        # is a directory
        ...
        # testdata to write a dummy file to the directory
