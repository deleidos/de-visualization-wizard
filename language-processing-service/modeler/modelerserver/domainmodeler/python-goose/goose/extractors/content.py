...
class ContentExtractor(BaseExtractor):
    ...
    def calculate_best_node(self):
        ...
        for node in nodes_with_text:
            ...
            text_node = self.parser.getText(node)
            word_stats = self.stopwords_class(language=self.get_language()).get_stopword_count(text_node)
            # upscore = int(word_stats.get_stopword_count() + boost_score) # ORG
            upscore = float(word_stats.get_stopword_count() + boost_score)
            ...
            if parent_parent_node is not None:
                self.update_node_count(parent_parent_node, 1.0)
                self.update_score(parent_parent_node, upscore / 2.0)
