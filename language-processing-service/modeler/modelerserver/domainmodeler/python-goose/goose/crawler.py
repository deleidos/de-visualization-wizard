...
class Crawler(object):
    ...
    def crawl(self, crawl_candidate):
        ...
        print("goose: crawler: crawl: self.article.title : ", self.article.title)
        ...
        print("goose: crawler: crawl: self.article.doc : ", self.cleaner.clean(),
              " self.article.top_node :", self.article.top_node
              )
        ...
        self.ascii_only()
        # return the article
        print ("\ngoose: crawler: crawl: self.article.title : ", self.article.title)
        return self.article

    def ascii_only(self):
        # remove non ascii
        try:
            self.article.cleaned_text = self.article.cleaned_text.encode('ascii', errors='ignore').decode('ascii', errors='ignore')
            self.article.cleaned_text = self.remove_punctuation(self.article.cleaned_text)
            self.article.title = self.article.title.encode('ascii', errors='ignore').decode('ascii', errors='ignore')
